...
public class RagdollModel extends ComplexObstacle {
    ...
    /**
     * Object to define revolution joints
     */
    private RevoluteJointDef revJointDef;
    ...
    protected void init() {
        ...
        // TORSO
        part = makePart(PART_BODY, PART_NONE, getX(), getY(), "torso");
        part.setFixedRotation(true);

        // HEAD
        makePart(PART_HEAD, PART_BODY, 0, TORSO_OFFSET, "head");

        // ARMS
        makePart(PART_LEFT_ARM, PART_BODY, -ARM_XOFFSET, ARM_YOFFSET, "leftArm");
        part = makePart(PART_RIGHT_ARM, PART_BODY, ARM_XOFFSET, ARM_YOFFSET, "rightArm");
        part.setAngle((float) Math.PI);

        // FOREARMS
        makePart(PART_LEFT_FOREARM, PART_LEFT_ARM, -FOREARM_OFFSET, 0, "leftForearm");
        part = makePart(PART_RIGHT_FOREARM, PART_RIGHT_ARM, FOREARM_OFFSET, 0, "rightForearm");
        part.setAngle((float) Math.PI);

        // THIGHS
        makePart(PART_LEFT_THIGH, PART_BODY, -THIGH_XOFFSET, -THIGH_YOFFSET, "leftThigh");
        makePart(PART_RIGHT_THIGH, PART_BODY, THIGH_XOFFSET, -THIGH_YOFFSET, "rightThigh");

        // SHINS
        makePart(PART_LEFT_SHIN, PART_LEFT_THIGH, 0, -SHIN_OFFSET, "leftShin");
        makePart(PART_RIGHT_SHIN, PART_RIGHT_THIGH, 0, -SHIN_OFFSET, "rightShin");
     ...
     */
    private BoxObstacle makePart(int part, int connect, float x, float y, String partName) {
    ...
    protected boolean createJoints(World world) {
        ...
        float lowerAngle = (float) (-0.5 * Math.PI);
        float upperAngle = (float) (0.5 * Math.PI);
        createRevJoint(world, PART_HEAD, PART_BODY, 0, -TORSO_OFFSET / 2, 0, TORSO_OFFSET / 2, lowerAngle, upperAngle);
        createRevJoint(world, PART_LEFT_ARM, PART_BODY, ARM_XOFFSET / 2, 0, -ARM_XOFFSET / 2, ARM_YOFFSET, lowerAngle, upperAngle);
        createRevJoint(world, PART_LEFT_ARM, PART_LEFT_FOREARM, -FOREARM_OFFSET / 2, 0, FOREARM_OFFSET / 2, 0, lowerAngle, upperAngle);
        createRevJoint(world, PART_RIGHT_ARM, PART_BODY, ARM_XOFFSET / 2, 0, ARM_XOFFSET / 2, ARM_YOFFSET, (float) (-1.5 * Math.PI), (float) (1.5 * Math.PI));
        createRevJoint(world, PART_RIGHT_ARM, PART_RIGHT_FOREARM, -FOREARM_OFFSET / 2, 0, FOREARM_OFFSET / 2, 0, lowerAngle, upperAngle);
        createRevJoint(world, PART_BODY, PART_LEFT_THIGH, -THIGH_XOFFSET, -THIGH_YOFFSET / 2, 0, THIGH_YOFFSET / 2, lowerAngle, upperAngle);
        createRevJoint(world, PART_LEFT_SHIN, PART_LEFT_THIGH, 0, SHIN_OFFSET / 2, 0, -SHIN_OFFSET / 2, lowerAngle, upperAngle);
        createRevJoint(world, PART_BODY, PART_RIGHT_THIGH, THIGH_XOFFSET, -THIGH_YOFFSET / 2, 0, THIGH_YOFFSET / 2, lowerAngle, upperAngle);
        createRevJoint(world, PART_RIGHT_SHIN, PART_RIGHT_THIGH, 0, SHIN_OFFSET / 2, 0, -SHIN_OFFSET / 2, lowerAngle, upperAngle);
        //#endregion
    ...
    /**
     * @param w          Box2D world
     * @param bodyA      index of first body to connect
     * @param bodyB      index of second body to connect
     * @param anchorAX   x-axis offset of first body
     * @param anchorAY   y-axis offset of first body
     * @param anchorBX   x-axis offset of second body
     * @param anchorBY   y-axis offset of second body
     * @param lowerAngle lower limit angle on the rev joint arc
     * @param upperAngle upper limit angle on the rev joint arc
     */
    private void createRevJoint(World w, int bodyA, int bodyB, float anchorAX, float anchorAY, float anchorBX, float anchorBY, float lowerAngle, float upperAngle) {
        revJointDef.bodyA = bodies.get(bodyA).getBody();
        revJointDef.bodyB = bodies.get(bodyB).getBody();
        revJointDef.localAnchorA.set(anchorAX, anchorAY);
        revJointDef.localAnchorB.set(anchorBX, anchorBY);
        revJointDef.lowerAngle = lowerAngle;
        revJointDef.upperAngle = upperAngle;
        joints.add(w.createJoint(revJointDef));
    }
}